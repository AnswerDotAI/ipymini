...
try:
    ...
    _EXPERIMENTAL_COMPLETIONS_AVAILABLE = False
...
def _thread_local_get_ipython():
    ...
    if shell is not None:
        return shell
    return _IO_STATE._orig_get_ipython()
...
class MiniDebugpyClient:
    ...
    def _handle_response(self, msg: dict) -> None:
        ...
        if isinstance(req_seq, int):
            with self._pending_lock:
                waiter = self._pending.get(req_seq)
    ...
    def wait_for_response(self, req_seq: int, waiter: queue.Queue, timeout: float = 10.0) -> dict:
        try:
            ...
            raise TimeoutError("timed out waiting for debugpy response") from exc
        finally:
            with self._pending_lock:
                self._pending.pop(req_seq, None)
        return reply

    def wait_initialized(self, timeout: float = 5.0) -> bool:
...
class MiniDebugger:
    ...
    def process_request(self, request: dict) -> tuple[dict, list[dict]]:
        ...
        if command == "attach":
            ...
            if not self.just_my_code:
                arguments["debugOptions"] = ["DebugStdLib"]
            ...
            if self.client.wait_initialized(timeout=10.0):
                config = {"type": "request", "seq": self.client.next_internal_seq(), "command": "configurationDone"}
                try:
                    self.client.send_request(config, timeout=10.0)
                except TimeoutError:
                    pass
    ...
    def trace_current_thread(self) -> None:
        if not _DEBUGPY_AVAILABLE or not self.started:
            return
    ...
    def _inspect_variables(self, request: dict) -> dict:
        ip = get_ipython()
        if ip is None:
            return self._response(request, False, body={"variables": []}, message="no ipython")
        variables = []
    ...
    def _rich_inspect_variables(self, request: dict) -> dict:
        ...
        ip = get_ipython()
        if ip is None:
            return self._response(request, False, body={"data": {}, "metadata": {}}, message="no ipython")
        ...
        if self.stopped_threads and args.get("frameId") is not None:
            ...
            try:
                ...
                return self._response(request, False, body={"data": {}, "metadata": {}}, message="timeout")
            if reply.get("success"):
                try:
                    repr_data, repr_metadata = eval(reply.get("body", {}).get("result", ""), {}, {})
                except Exception:
    ...
    def _copy_to_globals(self, request: dict) -> dict:
        ...
        try:
            ...
            return self._response(request, False, body={}, message="timeout")
        return reply
...
class MiniCommManager:
    ...
    def register_target(self, name: str, handler: Callable[[str, dict], None]) -> None:
...
def get_comm_manager() -> MiniCommManager:
...
def _murmur2_x86(data: str, seed: int) -> int:
    m = 0x5BD1E995
...
def _debug_tmp_directory() -> str:
    return os.path.join(tempfile.gettempdir(), f"ipymini_{os.getpid()}")


def _debug_hash_seed() -> int:
    return 0xC70F6907
...
def _debug_file_name(code: str) -> str:
    ...
    if cell_name is None:
        name = _murmur2_x86(code, _debug_hash_seed())
...
class KernelBridge:
    def __init__(
        ...
        *,
        ...
        def _code_name(raw_code: str, transformed_code: str, number: int) -> str:
            return _debug_file_name(raw_code)
        ...
        def _set_next_input(text: str, replace: bool = False) -> None:
            payload = {"source": "set_next_input", "text": text, "replace": bool(replace)}
    ...
    def set_stream_sender(self, sender: Callable[[str, str], None] | None) -> None:
        self._stream_sender = sender

    def _emit_stream(self, name: str, text: str) -> None:
        if self._stream_live and self._stream_sender is not None and text:
            self._stream_sender(name, text)
    ...
    def comm_info(self) -> dict:
        return {"status": "ok", "comms": _COMM_MANAGER.comm_info()}
