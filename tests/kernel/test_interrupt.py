...
async def test_interrupt_request_features():
    async with mini_kernel() as (km, kc):
        for _ in range(2):
            mid = str(uuid4())
            c = kc.reply("import asyncio; await asyncio.sleep(5)", timeout=1.5, msg_id=mid)
    ...
    async with mini_kernel() as (km, kc):
        for use_control_channel in [False, True]:
            mid = str(uuid4())
            c = kc.reply("import time; time.sleep(1)", timeout=10, msg_id=mid)
    ...
    async with mini_kernel() as (_, kc):
        mid = str(uuid4())
        c = kc.reply("import time; time.sleep(5); print('finished')", timeout=2, msg_id=mid)
        await wait_status(kc, "busy")
        await kc.interrupt()
        await _assert_interrupt(kc, c, timeout=2)

        mid = str(uuid4())
        c = kc.reply("import asyncio; await asyncio.sleep(5)", timeout=2, msg_id=mid)
        await wait_status(kc, "busy")
        await kc.interrupt()
        await _assert_interrupt(kc, c, timeout=2)

        mid = str(uuid4())
        c = kc.reply("while True: pass", timeout=2, msg_id=mid)
        await wait_status(kc, "busy")
        await kc.interrupt()
        await _assert_interrupt(kc, c, timeout=2)
    ...
    async with mini_kernel() as (_, kc):
        mid = str(uuid4())
        c = kc.reply("import time; time.sleep(1); print('finished')", timeout=2, msg_id=mid)
